...
void midpointellipse(int centerx, int centery, int radiusx, int radiusy) {
	vector<tuple<float, float>> temppoints;
	float x = 0;
	float y = radiusy;
	float D1 = (radiusy*radiusy) - (radiusx*radiusx*radiusy) + (.25*radiusx*radiusx);
	float dx = 2 * radiusy*radiusy*x;
	float dy = 2 * radiusx*radiusx*y;
	//upper
	while (dx < dy) {
		temppoints.push_back(make_tuple(x+centerx, y+centery));
		temppoints.push_back(make_tuple(-x + centerx, y + centery));
		temppoints.push_back(make_tuple(x + centerx, -y + centery));
		temppoints.push_back(make_tuple(-x + centerx, -y + centery));
		if (D1 < 0) {
	...
	//lower
	float D2 = ((radiusy*radiusy)*((x + .5)*(x + .5))) + ((radiusx*radiusx)*((y - 1)*(y - 1))) - (radiusx*radiusx*radiusy*radiusy);
	while (y >= 0) {
		temppoints.push_back(make_tuple(x + centerx, y + centery));
		temppoints.push_back(make_tuple(-x + centerx, y + centery));
		temppoints.push_back(make_tuple(x + centerx, -y + centery));
	...
	for (tuple<float, float> t : temppoints) {
		ellipses.push_back(make_tuple(round(get<0>(t)), round(get<1>(t))));
	}
}
void display() {
...
void keyBoard(unsigned char key, int x, int y) {
	//draw a circle
	if (key == 'R' || key == 'r') {
		midpointellipse(x, y, 50, 50);
	}
	//draw an ellipse
	if (key == 'E' || key == 'e') {
		midpointellipse(x, y, 50, 100);
	}
	if (key == 'W' || key == 'w') {
		ellipses.clear();
	}
